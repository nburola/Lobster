...
```{r}
#Making graphs of counts at each site each year for lobsters and traps
...
#graphs of avg counts for lobsters and traps

lobster_avg<-lobster_size_tidy %>% 
  select("year", "site", "size") %>% 
  group_by(year, site) %>% 
  summarize(mean = mean(size)) 
 
avg_graph <- lobster_avg %>% 
  ggplot(aes(x = year, y = mean))+
  geom_col()+
  facet_wrap(~site)
avg_graph

#uses mean for each year
trap_avg<-lobster_trap %>% 
  filter(site == "AQUE" | site == "CARP" | site == "IVEE" | site == "MOHK" |site == "NAPL") %>% 
  select("year", "site", "traps") %>% 
  group_by(year, site) %>% 
  summarize("mean" = mean(traps)) 

#uses mean trap from surveys each year
trap_avg2 <- trap_count %>% 
  group_by(year, site) %>% 
  summarize(mean = mean(traps))
